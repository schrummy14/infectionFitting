...
def showDailyChange(t,I,D):
    ...
    dI[1:] = I[1:] - I[:-1]
    dI[dI < 0] = 0
    plt.plot(t-t[0],dI,'ko',label="Daily Change")
    mdI = running_mean(dI,numDays)
    plt.plot(t-t[0],mdI,'k-',linewidth=2.5,label="%i Day Running Average"%numDays)
    plt.xlabel('Days Since First Death')
    plt.ylabel('Daily Additional Infected')
    plt.legend()
    ...
    dD[1:] = D[1:] - D[0:-1]
    dD[dD < 0] = 0
    plt.plot(t-t[0],dD,'ko',label="Daily Change")
    mdD = running_mean(dD,numDays)
    plt.plot(t-t[0],mdD,'k-',linewidth=2.5,label="%i Day Running Average"%numDays)
    plt.xlabel('Days Since First Death')
    plt.ylabel('Daily Additional Deaths')
    plt.legend()
